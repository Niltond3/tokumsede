...
type CallbackNodeType<T> = (Item: T) => ReactNode
...
export type TextFieldCaretButonProps = ButtonHTMLAttributes<HTMLButtonElement> & {
  diretion: 'L' | 'R'
}
...
export type DropdownMenuProps = DropdownContentProps & {
  portal?: boolean
}

export type DropdownProps = DropdownContentProps &
  DropdownMenuProps & {
    buttonIcon?: IconsKey
...
export type DropdownButtonProps = FragmentProps & {
  buttonIcon?: IconsKey
  openIndicator?: boolean
}

export type DropdownRadioGroupProps = FragmentProps & {
  onValueChange?: (value: string) => void
  value?: string
}

export type DropdownRadioItemProps = FragmentProps & {
  value: string
  onSelect?: (event: Event) => void
}

/// //
export type DropdownItemStateType<T> = DropdownDefaultProps<T>[] | DropdownDefaultProps<T>

type DropdownItemProps<T> = {
  onSelect?: () => void
  closeMenu?: () => void
  setSelect?: Dispatch<SetStateAction<DropdownItemStateType<T>>>
}
...
type KabanMarketplaceActions<T> = {
  onSelect?: () => void
  closeMenu?: () => void
  setSelect?: Dispatch<SetStateAction<KabanMarketplaceStateType<T>>>
}
...
export type KabanItemForSaleHandleEventProps = {
  ...
  setValues: Dispatch<SetStateAction<KabanItemForSaleValueStateType>>
}
...
export type KabanItemForSaleBodyProps = Omit<KabanItemForSaleComponnetsProps, 'label'> & {
  ...
  handleValue: {
    ...
    handleKeyboardChangeQuantity: (event: ChangeEvent<HTMLInputElement>) => void
  }
}
...
export type KabanItemForSaleHandleSelectProductProps = {
  event: React.MouseEvent<HTMLButtonElement, MouseEvent>
  setSelect?: Dispatch<
    SetStateAction<DropdownItemStateType<KabanProductType & KabanCurrentValueProps>>
  >
  values: KabanItemForSaleValueStateType
